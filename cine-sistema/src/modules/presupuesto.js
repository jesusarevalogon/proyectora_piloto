  ...
  return `
    <div class="grid">
      ...
      <div class="card">
        ...
        <hr class="hr" />
        <p class="muted">
          Adjuntos (V1): el bloque existe, pero Storage se integra en V2.<br/>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="form-grid">
            ...
            <label>
              <span>COT (V1)</span>
              <input id="bgCotFile" type="file" />
              <div class="muted" id="bgCotName" style="margin-top:6px;"></div>
            </label>

            <label>
              <span>CARTA (V1)</span>
              <input id="bgCartaFile" type="file" />
              <div class="muted" id="bgCartaName" style="margin-top:6px;"></div>
            </label>
          </div>
...
export function bindPresupuestoEvents() {
  ...
  const inpCotFile = document.getElementById("bgCotFile");
  const inpCartaFile = document.getElementById("bgCartaFile");
  const cotName = document.getElementById("bgCotName");
  const cartaName = document.getElementById("bgCartaName");
  ...
  // Adjuntos V1 (solo UI)
  inpCotFile.addEventListener("change", () => {
    cotName.textContent = inpCotFile.files?.[0]?.name ? `Archivo: ${inpCotFile.files[0].name}` : "";
  });
  inpCartaFile.addEventListener("change", () => {
    cartaName.textContent = inpCartaFile.files?.[0]?.name ? `Archivo: ${inpCartaFile.files[0].name}` : "";
  });
  ...
  function normalizeItem(it) {
    ...
    let formaPago = applyFormaRules(entidad, (it.formaPago || "EFECTIVO").toUpperCase());
  ...
  function renderTable() {
    ...
    items.forEach((it) => {
      ...
      tr.innerHTML = `
        ...
        <td>${escapeHtml(it.cot)}</td>
        <td>${escapeHtml(it.carta)}</td>
      `;
  ...
  function saveModal() {
    ...
    const cotVal = inpCotFile.files?.[0]?.name ? "Adj." : "no";
    const cartaVal = inpCartaFile.files?.[0]?.name ? "Adj." : "no";
    ...
    items[idx] = normalizeItem({
      ...
      iva,
      total,
      cot: cotVal !== "no" ? cotVal : items[idx].cot,
      carta: cartaVal !== "no" ? cartaVal : items[idx].carta,
      updatedAt: Date.now(),
    });
  ...
  function downloadCSV() {
    const headers = [
      ...
      "monto","cantidad","plazo","subtotal","iva","total","cot","carta"
    ];
  ...
  function parseBulkText(text) {
    ...
    for (let i = start; i < rows.length; i++) {
      ...
      // aplicar reglas:
      let formaPago = applyFormaRules(entidad, formaRaw || "EFECTIVO");
