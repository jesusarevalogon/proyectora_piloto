...
function readItemsFromLocalStorage() {
  ...
  const items = arr.map((it, i) => {
    ...
    const entidad =
      entidadRaw === "INTERNO" ? "PROPIAS" :
      entidadRaw === "PROPIA" ? "PROPIAS" :
      entidadRaw;
...
function readItemsFromDOM() {
  ...
  const presupuestoSection =
    document.querySelector('#app [data-route="presupuesto"]') ||
...
function aportacionesFrom(row) {
  ...
  if (e === "FOCINE") { out.focine_efectivo = val; return out; }
  if (e === "CENTRO") { out.centro_especie = val; return out; }
  if (e === "PROPIAS") {
...
function buildPrintableHTML(rows, projectName, opts = {}) {
  ...
  const grand = {
    ...
    focine_efectivo: 0, centro_especie: 0,
    propias_efectivo: 0, propias_especie: 0,
    terceros_efectivo: 0, terceros_especie: 0,
  };
...
/** =========================================================
 *  ✅ NUEVO: Exportar a PDF EN MEMORIA (Uint8Array)
 *  - NO depende de window.html2pdf
...
async function htmlToPdfBytes(html) {
  ...
  iframe.style.top = "0";
  iframe.style.width = "1400px";   // ancho grande para capturar bien la tabla
  ...
  // asegura fonts/layout
  try { await doc.fonts?.ready; } catch {}

  const target = doc.body;

  // canvas del contenido completo
  ...
  if (fullImgH <= usableH) {
    ...
    // múltiples páginas: cortar canvas en rebanadas
    const sliceCanvas = document.createElement("canvas");
    const sliceCtx = sliceCanvas.getContext("2d");

    // altura en px que corresponde a una página en pt
